...
// Keyboard set
const KEYBOARD = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789`~!@#$%^&*()-_=+[{]}\\|;:'\",<.>/? ".split('');
function randomChar() {
...
function insertChar(char, ts, cb) {
  db.run('INSERT INTO output (char, ts) VALUES (?, ?)', [char, ts], function (err) {
    if (err) {
      console.error('DB insert error:', err);
      return cb && cb(err);
    }
    cb && cb(null, { id: this.lastID, char, ts });
  });
}
      ...
      out.on('finish', () => {
        ...
        db.run('DELETE FROM output WHERE id <= ?', [lastId], function (delErr) {
          if (delErr) {
            ...
            // Note: we left the file on disk; decide later whether to delete file on error
          } else {
...
setInterval(() => {
  ...
  insertChar(ch, ts, (err, row) => {
    if (!err && row) {
      ...
      // If too many entries, archive oldest to keep main DB <= MAX_KEEP
